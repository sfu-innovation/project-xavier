...
var formatters = {
    ...
    'c': function(d, locale, t, l) {
        return format(d, locale.dateTimeFormat, t, l);
    },
    'C': function(d, locale, t, l) {
    ...
    'r': function(d, locale, t, l) {
        return format(d, '%I:%M:%S %p', t, l);
    },
    'R': function(d, locale, t, l) {
        return format(d, '%H:%M', t, l);
    },
    'S': function(d, locale) {
        return leadingZero(d.getSeconds());
    },
    'T': function(d, locale, t, l) {
        return format(d, '%H:%M:%S', t, l);
    },
    'u': function(d, locale) {
        return d.getDay()+1;
    },
    ...
    'x': function(d, locale, t, l) {
        return format(d, locale.dateFormat, t, l);
    },
    'X': function(d, locale, t, l) {
        return format(d, locale.timeFormat, t, l);
    },
    'y': function(d, locale) {
...
function format(d, format, timezone, locale) {
...
function formatAgo(d, timezone, baseTime, locale) {
    ...
    if (delta < minute) {
        ...
        var _format = n == 1 ? (ad > 0 ? l.secondAgo : l.secondIn) : (ad > 0 ? l.secondsAgo : l.secondsIn);
        return _format.replace('%t', n);
    } else if (delta < hour) {
        var n = Math.round(delta/minute);
        var _format = n == 1 ? (ad > 0 ? l.minuteAgo : l.minuteIn) : (ad > 0 ? l.minutesAgo : l.minutesIn);
        return _format.replace('%t', n);
    } else if (delta < day) {
        var n = Math.round(delta/hour);
        var _format = n == 1 ? (ad > 0 ? l.hourAgo : l.hourIn) : (ad > 0 ? l.hoursAgo : l.hoursIn);
        return _format.replace('%t', n);
    } else if (delta < week) {
        return format(d, '%A', timezone, locale);
    } else if (d.getYear() == baseTime.getYear()) {
    	//'%B %e%k at %i:%M%P'
        return format(d, '%B %e%k', timezone, locale);
    } else {
    	//'%B %e%k, %Y at %i:%M%P'
        return format(d, '%B %e%k, %Y', timezone, locale);
    }
};